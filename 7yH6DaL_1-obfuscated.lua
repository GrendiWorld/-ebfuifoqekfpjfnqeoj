...
client.set_event_callback("aim_hit", function(e)
    if not ui.get(hitlog_ref) or not entity.is_enemy(e.target) then return end
    client.delay_call(0, function()
        ...
        client.color_log(0, 255, 150, string.format("[resolver] Hit %s's %s for %d dmg (%d%% hc, bt=%dt) [hp: %d]%s", name, hg, dmg, hc, bt, hp, prefix))
    end)
end)

client.set_event_callback("aim_miss", function(e)
    if not ui.get(hitlog_ref) or not e.target then return end
    local target = e.target
    if not entity.is_enemy(target) then return end
    local name = entity.get_player_name(target)
    ...
    if e.aimpoint then
        ...
        if head:dist(ap) < 8 then hg = "head"
        elseif chest:dist(ap) < 10 then hg = "chest"
        elseif stomach:dist(ap) < 10 then hg = "stomach"
        elseif arm_l:dist(ap) < 8 then hg = "arm"
        elseif leg_l:dist(ap) < 10 then hg = "leg"
        end
    end

    local reason_map = {
        ["spread"] = "spread", ["occlusion"] = "wall", ["prediction error"] = "pred err",
        ["death"] = "death", ["resolver"] = "resolver", ["correction"] = "resolver",
        ["extrapolation"] = "extrap", ["unregistered shot"] = "unreg"
    }
    ...
    local prefix = side and side ~= "center" and (" — resolved " .. side:upper()) or ""
    client.color_log(255, 100, 100, string.format("[resolver] Missed %s's %s due to %s (hc=%d%%, bt=%dt)%s", name, hg, reason, hc, bt, prefix))
end)
