...
client.set_event_callback(
    "aim_hit",
    function(e)
        if not ui.get(hitlog_ref) or not entity.is_enemy(e.target) then
            return
        end
        client.delay_call(
            0,
            function()
                ...
                local side = resolved_side[target]

                local prefix = side and side ~= "center" and (" — resolved " .. side:upper()) or ""
                client.color_log(
                    0,
                    255,
                    150,
                    string.format(
                        "[resolver] Hit %s's %s for %d dmg (%d%% hc, bt=%dt) [hp: %d]%s",
                        name,
                        hg,
                        dmg,
                        hc,
...
client.set_event_callback(
    "aim_miss",
    function(e)
        if not ui.get(hitlog_ref) or not e.target then
            return
        end
        local target = e.target
        ...
        local hg = "body"
        if e.aimpoint then
            local ap = vector(e.aimpoint.x, e.aimpoint.y, e.aimpoint.z)
            local head = vector(entity.hitbox_position(target, 0))
            local chest = vector(entity.hitbox_position(target, 2))
            local stomach = vector(entity.hitbox_position(target, 3))
            local arm_l = vector(entity.hitbox_position(target, 4))
            local leg_l = vector(entity.hitbox_position(target, 6))

            if head:dist(ap) < 8 then
                hg = "head"
            elseif chest:dist(ap) < 10 then
                hg = "chest"
            elseif stomach:dist(ap) < 10 then
                hg = "stomach"
            elseif arm_l:dist(ap) < 8 then
                hg = "arm"
            elseif leg_l:dist(ap) < 10 then
                hg = "leg"
            end
        end
        ...
        local reason = reason_map[e.reason] or e.reason or "unknown"
        local hc = e.hitchance or 0
        local bt = e.backtrack or 0
        local side = resolved_side[target]
        ...
        client.color_log(
            ...
            100,
            100,
            string.format("[resolver] Missed %s's %s due to %s (hc=%d%%, bt=%dt)%s", name, hg, reason, hc, bt, prefix)
        )
    end
)
...
local native_GetNetChannelInfo,
    GetRemoteFramerate,
    native_GetTimeSinceLastReceived,
    native_GetAvgChoke,
    native_GetAvgLoss,
    native_IsLoopback,
    GetAddress
...
local cvar_game_mode, cvar_game_type, cvar_fps_max, cvar_fps_max_menu =
    cvar.game_mode,
    cvar.game_type,
    cvar.fps_max,
    cvar.fps_max_menu
...
local window =
    ...
    local function g(h, i, j, k, l, m, n, o, p)
        p = p or 1
        b(h, i, j, p, l, m, n, o)
        b(h, i + k - p, j, p, l, m, n, o)
        b(h, i + p, p, k - p * 2, l, m, n, o)
        b(h + j - p, i + p, p, k - p * 2, l, m, n, o)
    end
    local function q(h, i, j, k, r, s, t, u, v, w, x, y, z, p)
        p = p or 1
        if z then
            b(h, i, p, k, r, s, t, u)
            b(h + j - p, i, p, k, v, w, x, y)
            c(h + p, i, j - p * 2, p, r, s, t, u, v, w, x, u, true)
            c(h + p, i + k - p, j - p * 2, p, r, s, t, u, v, w, x, u, true)
        else
            b(h, i, j, p, r, s, t, u)
            b(h, i + k - p, j, p, v, w, x, y)
            c(h, i + p, p, k - p * 2, r, s, t, u, v, w, x, y, false)
            c(h + j - p, i + p, p, k - p * 2, r, s, t, u, v, w, x, y, false)
        end
    end
    local A
    do
    ...
    local function G(H)
        if H.position == "fixed" then
            ...
            if H.top ~= nil then
                H.y = D(H.top, J)
            elseif H.bottom ~= nil then
                H.y = J - (H.h or 0) - D(H.bottom, J)
            end
        end
        ...
        if H.title_bar then
            K = (H.title ~= nil and select(2, renderer.measure_text(H.title_text_size, H.title)) or 13) + 2
            H.t_x, H.t_y, H.t_w, H.t_h = H.x, H.y, H.w, K
        end
        if H.border then
            g(h, i, j, k, 18, 18, 18, o)
            g(h + 1, i + 1, j - 2, k - 2, 62, 62, 62, o)
            g(h + 2, i + K + 1, j - 4, k - K - 3, 44, 44, 44, o, H.border_width)
            g(
                h + H.border_width + 2,
                i + K + H.border_width + 1,
                j - H.border_width * 2 - 4,
                k - K - H.border_width * 2 - 3,
                62,
                62,
                62,
                o
            )
            H.i_x = H.i_x + H.border_width + 3
            H.i_y = H.i_y + H.border_width + 3
            H.i_w = H.i_w - (H.border_width + 3) * 2
            H.i_h = H.i_h - (H.border_width + 3) * 2
            H.t_x, H.t_y, H.t_w = H.x + 2, H.y + 2, H.w - 4
            K = K - 1
        end
        if K > 1 then
            c(H.t_x, H.t_y, H.t_w, K, 56, 56, 56, o, 44, 44, 44, o, false)
            if H.title ~= nil then
                local l, m, n, o = unpack(H.title_text_color)
                o = o * H.a / 255
                renderer.text(
                    H.t_x + 3,
                    H.t_y + 2,
                    ...
                    n or 255,
                    o or 255,
                    (H.title_text_size or "") .. (H.title_text_flags or ""),
                    0,
                    tostring(H.title)
                )
            end
        ...
        if H.gradient_bar then
            ...
            if H.background then
                L = 1
                local M, N = 16, 25
                b(H.i_x + 1, H.i_y, H.i_w - 2, 1, M, M, M, o)
                b(H.i_x + 1, H.i_y + 3, H.i_w - 2, 1, N, N, N, o)
                for O = 0, 1 do
                    c(H.i_x + (H.i_w - 1) * O, H.i_y, 1, 4, M, M, M, o, N, N, N, o, false)
                end
            end
            do
                local h, i, P = H.i_x + L, H.i_y + L, 1
                local Q, R = e((H.i_w - L * 2) / 2), f((H.i_w - L * 2) / 2)
                for O = 1, 2 do
                    c(h, i, Q, 1, 59 * P, 175 * P, 222 * P, o, 202 * P, 70 * P, 205 * P, o, true)
                    c(h + Q, i, R, 1, 202 * P, 70 * P, 205 * P, o, 201 * P, 227 * P, 58 * P, o, true)
                    i, P = i + 1, P * 0.5
                end
            end
            H.i_y = H.i_y + 2 + L * 2
        ...
        if H.draggable then
            local p = 7
            renderer.triangle(h + j - 1, i + k - p, h + j - 1, i + k - 1, h + j - p, i + k - 1, 62, 62, 62, o)
        end
        H.i_x = H.i_x + H.margin_left
        H.i_w = H.i_w - H.margin_left - H.margin_right
        H.i_y = H.i_y + H.margin_top
        H.i_h = H.i_h - H.margin_top - H.margin_bottom
    end
    local S = {}
    local T = {}
    local U = {}
    ...
    function U:set_inner_height(Z)
        ...
        Z = Z + self.margin_top + self.margin_bottom
        self.h = Z
    end
                ...
                for O = #ab, 1, -1 do
                    ...
                    if H.visible and H:is_in_window(a1, a2) then
                        ...
                        if a3 then
                            ...
                            if H.position == "fixed" then
                                local ah = H.left == nil and "right" or "left"
                                local ai = H.top == nil and "bottom" or "top"
                                local aj = {
                                    {ah, (ah == "right" and -1 or 1) * af},
                                    {ai, (ai == "bottom" and -1 or 1) * ag}
                                }
                                for O = 1, #aj do
                                    local ak, al = unpack(aj[O])
                                    local am = type(H[ak])
...
local watermark_items = {
    ...
    },
    {
        ...
        get_width = function(self, frame_data)
            ...
            local fps_max =
                globals.mapname() == nil and
                math.min(fps_max == 0 and 999 or fps_max, fps_max_menu == 0 and 999 or fps_max) or
                fps_max == 0 and 999 or
                fps_max
    ...
    },
    {
        ...
        get_width = function(self, frame_data)
            if not allow_unsafe_scripts then
                return
            end
            frame_data.local_player = frame_data.local_player or entity.get_local_player()
            if frame_data.local_player == nil then
            ...
            local frame_time, frame_time_std_dev, frame_time_start_time_std_dev =
                GetRemoteFramerate(frame_data.net_channel_info)
    ...
    },
    {
        ...
        get_width = function(self, frame_data)
            ...
            frame_data.is_loopback =
                frame_data.is_loopback == nil and native_IsLoopback(frame_data.net_channel_info) or
                frame_data.is_loopback
            local game_rules = entity.get_game_rules()
            frame_data.is_valve_ds =
                frame_data.is_valve_ds == nil and entity.get_prop(game_rules, "m_bIsValveDS") == 1 or
                frame_data.is_valve_ds
            local text
    ...
    },
    {
        ...
        get_width = function(self, frame_data)
            ...
            frame_data.is_valve_ds =
                frame_data.is_valve_ds == nil and entity.get_prop(entity.get_game_rules(), "m_bIsValveDS") == 1 or
                frame_data.is_valve_ds
    ...
    },
    {
        ...
        get_width = function(self, frame_data)
            ...
            return self.time_width + self.sep_width + self.time_width +
                (self.seconds and (self.sep_width + self.time_width) or 0)
        end,
        draw = function(self, x, y, w, h, r, g, b, a)
...
window.paint_callback = function()
    ...
    for i = 1, #items_drawn do
        ...
        if #items_drawn > i then
            renderer.rectangle(
                window.i_x + item.x + item.item_width + 4,
                window.i_y + 1,
                1,
                window.i_h - 1,
                210,
