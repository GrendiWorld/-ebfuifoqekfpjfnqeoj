...
local function get_hwid()
    local fs_interface = client.create_interface("filesystem_stdio.dll", "VBaseFileSystem011") or error("Не удалось создать интерфейс файловой системы")
...
local function run_auth_check()
    ...
    http.get("https://raw.githubusercontent.com/GrendiWorld/-ebfuifoqekfpjfnqeoj/main/auth1.txt", function(success, response)
...
local cals = {
	...
	wireframe = {
		...
		run_death = function(self, e)
			if not menu.options.vis.Wireframe:get() then return end
...
{"player_death", function(event)
		if cals.tt then cals.tt:run(event) end
		if cals.wireframe then cals.wireframe:run_death(event) end
	end},

	{"round_prestart", function()
		if cals.wireframe then cals.wireframe.players = {} end
	end},
	...
	{"setup_command", function(cmd) 
		...
		if cals.extended_bt then cals.extended_bt:run() end 
	end},
	...
	{"paint", function()
		...
		cals.dmgmarker:run()
		cals.ClanTag:run()
		if cals.wireframe then cals.wireframe:render() end
